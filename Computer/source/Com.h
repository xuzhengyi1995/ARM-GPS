...
public:
	...
	bool SetState(char *szSetStr)
	{
		if (IsOpen())
		{
			if (::GetCommState(_hCommHandle, &_DCB) != TRUE)
				return false;
			if (::BuildCommDCB(LPCWSTR(szSetStr), &_DCB) != TRUE)
				return false;
	...
	//----------------------------------Operations----------------------------------
	//打开串口 缺省 9600, 8, n, 1
	bool Open(CString &dwPort)
	{
		return Open(dwPort, 9600);
	}
	//打开串口 缺省 baud_rate, 8, n, 1
	bool Open(CString &dwPort, DWORD dwBaudRate)
	{
	...
	bool Open(CString &dwPort, char *szSetStr)
	{
		//if (dwPort < 1 || dwPort > 1024)
			//return false;
...
protected:
	...
	CString _szCommStr; //保存COM1类似的字符串
	...
	//绑定串口
	void BindCommPort(CString &dwPort)
	{
	...
	virtual DWORD ThreadFunc()
	{
		if (!::SetCommMask(_hCommHandle, _dwMaskEvent))
		{
			...
			MessageBox(NULL, LPCWSTR(szBuffer), L"Class cnComm", MB_OK);
			return 1;
		}
